...
ResonariumLookAndFeel::ResonariumLookAndFeel()
{
    ...
    setColour(accentColourId, juce::Colour(0xE000FFAE).brighter(0.2));
                                             ...
                                             const juce::Slider::SliderStyle, juce::Slider& slider)
{
    ...
    if (slider.isHorizontal())
    {
        ...
        {
            g.fillRect(juce::Rectangle<float>(float(rc.getX()), float(rc.getY()), sliderPos - rc.getX(),
                                              float(rc.getHeight())));
        }
    }
    else
    {
        g.fillRect(juce::Rectangle<float>(rc.getX() + 0.5f, sliderPos, rc.getWidth() - 1.0f,
                                          rc.getY() + (rc.getHeight() - sliderPos)));
    }
}

void ResonariumLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height, float sliderPos,
                                             const float rotaryStartAngleIn, const float rotaryEndAngle,
                                             juce::Slider& slider)
{
...

    {
        ...
        knob.addArc(rcI.getX(), rcI.getY(), rcI.getWidth(), rcI.getHeight(), angle - pi / 2, angle + pi / 2 - pi * 2,
                    false);
        knob.closeSubPath();
        if (slider.isEnabled())
        {
...
{
    auto c = b.findColour(b.getToggleState() ? juce::TextButton::textColourOnId : juce::TextButton::textColourOffId).
...
void ResonariumLookAndFeel::drawTooltip(juce::Graphics& g, const juce::String& text, int width, int height)
{
    ...
    juce::detail::LookAndFeelHelpers::layoutTooltipText(getDefaultMetricsKind(), text,
...
void ResonariumLookAndFeel::drawTextEditorOutline(juce::Graphics& g, int width, int height,
...
juce::Font ResonariumLookAndFeel::getLabelFont(juce::Label& label)
{
    if (true)
    {
        return defaultFont.withHeight(label.getFont().getHeight()).withExtraKerningFactor(
